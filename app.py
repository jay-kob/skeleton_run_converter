...
# File uploader with multiple file upload enabled
...
if uploaded_files:
    ...
    if len(uploaded_files) > MAX_FILES:
        ...
        output_files = {}  # Dictionary to store processed files
        for uploaded_file in uploaded_files:
            input_filename = uploaded_file.name.rsplit('.', 1)[0]
            
            # Proceed with processing each file as in the previous example
        ...
        excel_data.seek(0)
        output_files[input_filename] = excel_data

    # Display download buttons for each processed file
    st.write("## Download Processed Files")
    for filename, excel_data in output_files.items():
        st.download_button(
            label=f"Download {filename}_processed.xlsx",
            data=excel_data,
